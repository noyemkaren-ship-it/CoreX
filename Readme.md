...
**Главная фишка:** CoreX обрабатывает только свои команды (`import`, `use`ф, `Sys.`). Весь остальной C++ код проходит насквозь без изменений! Можно смешивать CoreX и C++ как угодно!

## СТОИТ ОБРАТИТЬ ВНИМАНИЯ ЧТО ЭТО ЯЗЫК ВЕДЬ, НО ОН ИСПОЛЬЗУЕТ g++ ДЛЯ КОМПИЛЯЦИИ И ОТТУДА КАК ПОБОЧНЫЙ ЭФЕКТ И ПОЯВИЛАСЬ ПОЛНАЯ ПОДДЕРЖКА C++ КОДОВ!
